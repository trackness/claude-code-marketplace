...
2. Sanitize: remove fenced code blocks (``` and ~~~), inline code spans (backticks), URLs (regex \bhttps?://\S+), and blockquote lines (lines starting with optional whitespace then >) — pasted/quoted material is not the user's own asking.
...
4. A sentence is a QUESTION if any of:
   a. it ends with `?`;
...
Composition: quote up to MAX_QUOTED_QUESTIONS = 5 detected question sentences, each truncated to MAX_QUOTE_CHARS = 200, whole reminder capped at MAX_REMINDER_CHARS = 3500 (headroom under the platform's 10k cap). Two template constants:
...
Delivery: default is quiet — print JSON {"hookSpecificOutput": {"hookEventName": "UserPromptSubmit", "additionalContext": <reminder>}}. If env var QUESTION_GUARD_VISIBLE is set to a truthy value (case-insensitive: 1, true, yes, on), print the reminder as plain stdout instead, so the user can watch it fire while tuning. Always exit 0.
